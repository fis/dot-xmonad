...
-- | Builds the default configuration. This function also registers the D-Bus name for the status updates.
zemConfig = do
  home <- getEnv "HOME"
  ...
  let conf = def { workspaces = workspaceNames
  ...
  return
    . navigation2DConfig conf
    . (`additionalKeys` customKeys conf dbus home)
